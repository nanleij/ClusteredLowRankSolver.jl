...
function ClusteredLowRankSolver.exact_solution(model::GenericModel{T}; FF=QQ, g=BigFloat(1), settings::RoundingSettings=RoundingSettings(), transformed=false, verbose=true) where T
    ...
    try
        ...
        for (k,x) in objs
            ...
            if length(idx) > 1
                idx_to_var_psd[idx[1]] = k
            else
                idx_to_var_free[idx] = k
            end
        end   
    catch
    end
    ...
    if transformed
        ...
        try
            problem = change_varkeys(problem, idx_to_var_psd, idx_to_var_free)
            esol = change_varkeys(esol, idx_to_var_psd, idx_to_var_free)
            transformations = Dict(idx_to_var_psd[i]=>m for (i,m) in transformations)
        catch 
        end
...
function change_varkeys(p::Problem, idx_to_var_psd, idx_to_var_free)
    cs = [change_varkeys(c, idx_to_var_psd, idx_to_var_free) for c in p.constraints]
    return Problem(p.maximize, change_varkeys(p.objective, idx_to_var_psd, idx_to_var_free), cs)
end

function change_varkeys(c::Constraint, idx_to_var_psd, idx_to_var_free)
    dct_mats = Dict(idx_to_var_psd[k]=>m for (k,m) in matrixcoeffs(c))
    dct_fv = Dict(idx_to_var_free[k]=>m for (k,m) in freecoeffs(c))
    return Constraint(c.constant, dct_mats, dct_fv, c.samples, c.scalings)
end

function change_varkeys(c::Objective,  idx_to_var_psd, idx_to_var_free)
    dct_mats = Dict(idx_to_var_psd[k]=>m for (k,m) in matrixcoeffs(c))
    dct_fv = Dict(idx_to_var_free[k]=>m for (k,m) in freecoeffs(c))
    return Objective(c.constant, dct_mats, dct_fv)
end

function change_varkeys(c::Union{PrimalSolution{T}, DualSolution{T}},  idx_to_var_psd, idx_to_var_free) where T
    dct_mats = Dict(idx_to_var_psd[k]=>m for (k,m) in matrixvars(c))
    if c isa PrimalSolution
        dct_fv = Dict(idx_to_var_free[k]=>m for (k,m) in freevars(c))
